...
export default function FoundationDiggingService() {
  ...
  const serviceData = {
    ...
    shortDescription: "Земляные работы для устройства ленточных и плитных фундаментов.",
  ...
  const seoData = servicesSeoData['kopka-pod-fundament' as keyof typeof servicesSeoData];
  const serviceSchema = generateServiceSchema(serviceData.title, serviceData.shortDescription);
  const breadcrumbSchema = generateBreadcrumbSchema([
    { name: 'Главная', url: '/' },
    { name: 'Услуги', url: '/services' },
    { name: serviceData.title, url: '/services/kopka-pod-fundament' }
  ]);
              ...
              <div style={{
                ...
                <div style={{ marginBottom: isMobile ? '30px' : '50px', textAlign: 'center' }}>
                  <p style={{
                    ...
                    От правильного выполнения этого этапа зависит надёжность всей опорной конструкции. В зависимости от определённого проектом типа фундамента, земляные работы могут предусматривать: Выкапывание траншеи под монолитную или сборную фундаментную ленту; Снятие плодородного слоя почвы для заливки железобетонной плиты; Разработку глубокого котлована для зданий с подвальными помещениями; Бурение или выкапывание ям для устройства столбчатых оснований. Также к разработке грунта относят планировку участка застройки и организацию подъездных путей.
                  </p>
                </div>
              </div>
            </div>
