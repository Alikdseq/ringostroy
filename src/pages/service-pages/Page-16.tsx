...
export default function MulchingService() {
  ...
  const serviceData = {
    title: "Покос травы мульчером",
    shortDescription: "Покос травы и кустарников мульчером для облагораживания территории.",
  ...
  const seoData = servicesSeoData['pokos-travy-mulcherom' as keyof typeof servicesSeoData];
  const serviceSchema = generateServiceSchema(serviceData.title, serviceData.shortDescription);
  const breadcrumbSchema = generateBreadcrumbSchema([
    { name: 'Главная', url: '/' },
    { name: 'Услуги', url: '/services' },
    { name: serviceData.title, url: '/services/pokos-travy-mulcherom' }
  ]);
              ...
              <div style={{
                ...
                <div style={{ marginBottom: isMobile ? '30px' : '50px', textAlign: 'center' }}>
                  <p style={{
                    ...
                    Покос травы мульчером — это использование специализированного оборудования для эффективного удаления растительности на обширных участках земли. Мульчер не только срезает траву, но и измельчает её до состояния мульчи и равномерно распределяет по поверхности земли, что ускоряет процесс разложения органических остатков.
                  </p>
                </div>
              </div>
            </div>
