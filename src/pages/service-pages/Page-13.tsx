...
export default function ExcavationService() {
  ...
  const serviceData = {
    title: "Копка котлована",
    shortDescription: "Профессиональная разработка котлованов любого размера для строительства фундаментов.",
  ...
  const seoData = servicesSeoData['kopka-kotlovana' as keyof typeof servicesSeoData];
  const serviceSchema = generateServiceSchema(serviceData.title, serviceData.shortDescription);
  const breadcrumbSchema = generateBreadcrumbSchema([
    { name: 'Главная', url: '/' },
    { name: 'Услуги', url: '/services' },
    { name: serviceData.title, url: '/services/kopka-kotlovana' }
  ]);
              ...
              <div style={{
                ...
                <div style={{ marginBottom: isMobile ? '30px' : '50px', textAlign: 'center' }}>
                  <p style={{
                    ...
                    Для разработки котлована используют специальную технику — экскаваторы. Они позволяют быстро и эффективно выполнить земляные работы. Основные этапы разработки котлована: Подготовка территории: удаление растительности, мусора и других препятствий на участке. Определение необходимой глубины котлована с учётом проектных параметров и целей строительства. Разметка границ и размеров будущего котлована. Проверка наличия в грунте труб, электрических сетей, канализации и других инженерных коммуникаций. Удаление верхнего слоя почвы и разрыхление грунта. Разработка котлована экскаватором с применением выбранного метода.
                  </p>
                </div>
              </div>
            </div>
