...
export default function PileDrillingService() {
  ...
  const serviceData = {
    title: "Бурение свай до 2.5 метров",
    shortDescription: "Бурение свайных отверстий до 2.5 метров для устройства свайного фундамента.",
  ...
  const seoData = servicesSeoData['burenie-svaj-do-2-5-metrov' as keyof typeof servicesSeoData];
  const serviceSchema = generateServiceSchema(serviceData.title, serviceData.shortDescription);
  const breadcrumbSchema = generateBreadcrumbSchema([
    { name: 'Главная', url: '/' },
    { name: 'Услуги', url: '/services' },
    { name: serviceData.title, url: '/services/burenie-svaj-do-2-5-metrov' }
  ]);
              ...
              <div style={{
                ...
                <div style={{ marginBottom: isMobile ? '30px' : '50px', textAlign: 'center' }}>
                  <p style={{
                    ...
                    Обычно пробное бурение осуществляется на глубину 1,7 метра. Для этого используют тонкую трубу на конце штанги, которой загоняют в отверстие от пробного завинчивания специальной сваи. Так берут пробу грунта, в который упёрлась пробная свая. Также бурение на глубину до 2 метров может проводиться при обустройстве набивных свай. В этом случае буровая установка разрабатывает скважину, в неё погружают арматурный каркас и заполняют выемку бетонной смесью, в результате чего в грунте создаётся полноценная железобетонная опора.
                  </p>
                </div>
              </div>
            </div>
