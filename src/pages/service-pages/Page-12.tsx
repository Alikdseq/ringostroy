...
export default function HydraulicHammerService() {
  ...
  const serviceData = {
    title: "Услуги гидромолота",
    shortDescription: "Разработка грунтов, разрушение бетона и скальных пород с помощью гидромолота.",
  ...
  const seoData = servicesSeoData['uslugi-gidromolota' as keyof typeof servicesSeoData];
  const serviceSchema = generateServiceSchema(serviceData.title, serviceData.shortDescription);
  const breadcrumbSchema = generateBreadcrumbSchema([
    { name: 'Главная', url: '/' },
    { name: 'Услуги', url: '/services' },
    { name: serviceData.title, url: '/services/uslugi-gidromolota' }
  ]);
              ...
              <div style={{
                ...
                <div style={{ marginBottom: isMobile ? '30px' : '50px', textAlign: 'center' }}>
                  <p style={{
                    ...
                    С помощью гидромолота выполняют различные работы, например: Вскрытие подземных коммуникаций; Демонтаж дорожных покрытий; Рыхление мёрзлого и каменистого грунта; Разрушение железобетонных конструкций. Аренда гидромолота позволяет избежать чрезмерных затрат на покупку, обслуживание и хранение оборудования, если оно не обеспечено постоянной загрузкой. Например, аренда колёсного гидромолота удобна для работ в городской застройке с ограниченной площадью и необходимостью маневрирования, а техника на гусеничном ходу подойдёт для площадок без покрытия, где важна повышенная проходимость и устойчивость.
                  </p>
                </div>
              </div>
            </div>
