...
export default function WasteRemovalService() {
  ...
  const serviceData = {
    title: "Вывоз строительного и любого мусора",
    shortDescription: "Своевременный вывоз строительного и бытового мусора с утилизацией.",
  ...
  const seoData = servicesSeoData['vyvoz-stroitelnogo-i-lyubogo-musora' as keyof typeof servicesSeoData];
  const serviceSchema = generateServiceSchema(serviceData.title, serviceData.shortDescription);
  const breadcrumbSchema = generateBreadcrumbSchema([
    { name: 'Главная', url: '/' },
    { name: 'Услуги', url: '/services' },
    { name: serviceData.title, url: '/services/vyvoz-stroitelnogo-i-lyubogo-musora' }
  ]);
              ...
              <div style={{
                ...
                <div style={{ marginBottom: isMobile ? '30px' : '50px', textAlign: 'center' }}>
                  <p style={{
                    ...
                    Для сбора и транспортировки строительного мусора требуется специальная лицензия и специальный электронный талон зарегистрированный в системе ОССиГ, при отсутствии которой можно получить штраф от контролирующих органов. Специализированные организации не только помогут избежать подобных проблем, но и организуют процесс вывоза максимально оперативно, так как имеют разрешения на работу в любых районах города, в том числе центральном, сотрудничают с большинством полигонов и компаний по переработке, знают нюансы и смогут оперативно организовать погрузку даже в наиболее труднодоступных местах. Вывоз отходов — услуга комплексная. Она предусматривает: Сбор (накопление) мусора в соответствии с действующими требованиями; Транспортировку до места утилизации; Утилизацию на полигонах или профильных предприятиях путем уничтожения или переработки. Транспорт для перевозки строительного мусора подбирается исходя из массы, габаритов и состава отходов. Хлам после незначительного ремонта можно вывезти в Газели с грузоподъемностью в 1,5 т и длиной кузова 3-4 метра. Старую мебель, двери, оконные рамы, лутки, карнизы и плинтуса влезут в фургон с вместимостью до 8 м3.
                  </p>
                </div>
              </div>
            </div>
