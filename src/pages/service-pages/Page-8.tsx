...
export default function TreeRemovalService() {
  ...
  const serviceData = {
    title: "Сруб и вывоз деревьев",
    shortDescription: "Профессиональная валка деревьев, спил сучьев и вывоз древесины с вашего участка.",
  ...
  const seoData = servicesSeoData['srub-i-vyvoz-derevev' as keyof typeof servicesSeoData];
  const serviceSchema = generateServiceSchema(serviceData.title, serviceData.shortDescription);
  const breadcrumbSchema = generateBreadcrumbSchema([
    { name: 'Главная', url: '/' },
    { name: 'Услуги', url: '/services' },
    { name: serviceData.title, url: '/services/srub-i-vyvoz-derevev' }
  ]);
              ...
              <div style={{
                ...
                <div style={{ marginBottom: isMobile ? '30px' : '50px', textAlign: 'center' }}>
                  <p style={{
                    ...
                    Сруб деревьев может осуществляться разными способами, например, целиком или по частям, со свободным падением веток или их спуском на верёвках. Вывоз порубочных остатков предполагает транспортировку спиленных веток, сучьев и других растительных отходов за пределы участка для сжигания или переработки/утилизации. Для проведения таких работ рекомендуется обратиться к профессиональным компаниям, которые имеют необходимую технику и опыт. Они смогут выполнить все этапы работы, включая подготовку участка, подъём на нужную высоту, уборку веток и стволов, распил деревьев, погрузку и вывоз брёвен, удаление пней и уборку участка.
                  </p>
                </div>
              </div>
            </div>
