...
export default function ConcreteAsphaltCuttingService() {
  ...
  const serviceData = {
    title: "Резка бетона и асфальта",
    shortDescription: "Резка и демонтаж бетонных конструкций и асфальтовых покрытий.",
  ...
  const seoData = servicesSeoData['rezka-betona-i-asfalta' as keyof typeof servicesSeoData];
  const serviceSchema = generateServiceSchema(serviceData.title, serviceData.shortDescription);
  const breadcrumbSchema = generateBreadcrumbSchema([
    { name: 'Главная', url: '/' },
    { name: 'Услуги', url: '/services' },
    { name: serviceData.title, url: '/services/rezka-betona-i-asfalta' }
  ]);
              ...
              <div style={{
                ...
                <div style={{ marginBottom: isMobile ? '30px' : '50px', textAlign: 'center' }}>
                  <p style={{
                    ...
                    Для бетона используют алмазные диски, алмазные коронки и алмазные канаты. Асфальтовое полотно, включающее в себя гранитный щебень и битум, режут дисками и коронками с алмазным напылением. Преимущества алмазной резки перед отбойными молотками и перфораторами — отсутствие вибрации, низкий уровень шума, высокая скорость работы и качественный рез. В дорожной отрасли алмазную резку применяют, например, для выравнивания края дороги с целью дальнейшей установки бордюров, удаления определённого слоя асфальтного покрытия и коррекции неровностей дорожного покрытия перед выполнением ямочного ремонта.
                  </p>
                </div>
              </div>
            </div>
